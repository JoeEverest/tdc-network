...
export const endorsementKeys = {
  ...
  canEndorse: (endorserId: string, userId: string, skillId: string) =>
    [...endorsementKeys.eligibility(), endorserId, userId, skillId] as const,
};
...
export const useCreateEndorsement = () => {
  ...
  return useMutation({
    ...
    onSuccess: (newEndorsement) => {
      ...
      queryClient.invalidateQueries(endorsementKeys.userGiven(endorserId));
...
export const useUpdateEndorsement = () => {
  ...
  return useMutation({
    ...
    }) => endorsementService.updateEndorsement(endorsementId, endorsementData),
