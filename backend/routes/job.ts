...
router.post("/", requireAuth, async (req: any, res: Response) => {
  try {
    ...
    for (const skill of requiredSkills) {
      if (!skill.skill || !skill.minRating) {
        return res.status(400).json({
          error:
            "Each required skill must have a skillId and a minRating between 1 and 10",
        });
      }
    }
...
router.put("/:jobId", requireAuth, async (req: any, res: Response) => {
  try {
    ...
    if (
      ...
      for (const skill of requiredSkills) {
        if (!skill.minRating || skill.minRating < 1 || skill.minRating > 10) {
          return res.status(400).json({
            error:
              "Each required skill must have a skillId and a minRating between 1 and 10",
          });
